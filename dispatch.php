...
<div class="row">
    <div class="col-lg-12">
        <div class="card custom-card overflow-hidden">
            <div class="card-body">
                <div>
                    <h6 class="card-title mb-1">Dispatches</h6>
                </div>
                    ...
                    <table id="exportexample" class="table table-bordered border-t0 key-buttons text-nowrap w-100" >
                        ...
                        <tr>
                            <?php   while($row = mysqli_fetch_assoc($rows)):?>
                                <td><?php echo $row["Dispatch_id"]?></td>
                                <td><?php echo $row["Dispatch_date"]?></td>
                                <td><?php echo $row["Dispatch_date"]?></td>
                                <td><button onclick="window.location.href='editdispatch.php?id=<?php echo $row['Dispatch_id']; ?>'" class="btn btn-warning btn-round">Edit</button></td>
                                <td><button onclick="window.location.href='deletedispatch.php?id=<?php echo $row['Dispatch_id']; ?>'" class="btn btn-danger btn-round">Delete</button></td>
                                </tr>
                            <?php  endwhile; ?>
                        <tbody>
                    </table>
